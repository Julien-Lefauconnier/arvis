...
def test_epsilon_increases_when_confidence_low():
    low = apply_confidence_control(
        ConfidenceControlInputs(0.1, 0.2, 1.0)
    )
    high = apply_confidence_control(
        ConfidenceControlInputs(0.9, 0.2, 1.0)
    )
...
def test_very_low_confidence_flag():
    out = apply_confidence_control(
        ConfidenceControlInputs(0.05, 0.2, 1.0)
    )
