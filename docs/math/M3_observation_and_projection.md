...
- normalization,
- boundedness,
- continuity under small metric perturbations.
...
- low sensitivity to transient spikes.
...
- margin to switching boundaries.
...
Validation focus:
- monotonicity with respect to uncertainty,
- boundedness,
- non-amplification.
...
- no inf,
- no unbounded growth,
- no out-of-domain mode.

---

### 7.3 Local Lipschitz Campaign

For each nominal observation \( o \), generate perturbations \( o+\delta o_i \) with controlled magnitude.
...
Acceptance criteria:
- bounded empirical local Lipschitz constant,
- no explosive ratios,
- stable distribution across the corpus.
...
- projection drift,
- mode flips,
- Lyapunov drift after projection.

Acceptance criteria:
- bounded degradation,
- no catastrophic amplification,
- rare mode flips away from boundaries.
...
- mode stability under repeated perturbation,
- minimal perturbation needed to trigger a mode change,
- empirical dwell-time compatibility over trajectories.

Acceptance criteria:
- clear margins away from boundaries,
- bounded flip frequency,
- no chattering under small perturbations.

---

### 7.6 Lyapunov Compatibility Campaign

For projected trajectories, compute:
...
and verify:
- finiteness,
- continuity over nearby samples,
- consistency with observer expectations.

Acceptance criteria:
- \( W_q \) always computable,
- no pathological jumps except admissible switching,
- observer outputs remain interpretable.
...
3. empirical local Lipschitz ratios remain below a chosen threshold on at least 99% of the validated corpus,
4. mode-flip rate under admissible perturbations remains below a chosen threshold away from boundaries,
...
Mode instability in the boundary region is admissible if explicitly tracked.
