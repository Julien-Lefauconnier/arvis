...
class PipelineBootstrapService:
    ...
    def run(
        ...
        pipeline.core = CognitiveCoreEngine(core_model=core_model)

        pipeline.hysteresis = ModeHysteresis()
        pipeline.exploration = ExplorationController()
        pipeline.regime_policy = CognitiveRegimePolicy()
        pipeline.regime_estimator = CognitiveRegimeEstimator()

        pipeline.epsilon_controller = IRGEpsilonController(
            adaptive_params=EpsAdaptiveParams(enabled=True)
        )

        pipeline.temporal_pressure = TemporalPressure()
        pipeline.temporal_regulation = TemporalRegulation()
        pipeline.action_policy = ActionPolicy()

        pipeline.conflict_evaluator = ConflictEvaluator(rules=default_conflict_rules())

        pipeline.observability = ObservabilityBuilder()
        pipeline.conflict_pressure_engine = ConflictPressureEngine()
        pipeline.coherence_observer = CoherenceObserver()
        pipeline.coherence_policy = CoherencePolicy()
        ...
        pipeline.tool_feedback_stage = ToolFeedbackStage()
        pipeline.tool_retry_stage = ToolRetryStage()
        pipeline.decision_stage = DecisionStage()
        pipeline.passive_stage = PassiveContextStage()
        pipeline.bundle_stage = BundleStage()
        pipeline.conflict_stage = ConflictStage()
        pipeline.core_stage = CoreStage()
        pipeline.regime_stage = RegimeStage()
        pipeline.temporal_stage = TemporalStage()
        pipeline.conflict_modulation_stage = ConflictModulationStage()
        pipeline.control_stage = ControlStage()
        pipeline.projection_stage = ProjectionStage()
        pipeline.gate_stage = GateStage()
        pipeline.control_feedback_stage = ControlFeedbackStage()
        pipeline.structural_risk_stage = StructuralRiskStage()
        pipeline.confirmation_stage = ConfirmationStage()
        pipeline.execution_stage = ExecutionStage()
        pipeline.action_stage = ActionStage()
        pipeline.intent_stage = IntentStage()
        pipeline.runtime_stage = RuntimeStage()

        pipeline.global_stability_observer = GlobalStabilityObserver()

        pipeline.quadratic_lyapunov_family = make_default_quadratic_family(dim=4)

        pipeline.quadratic_comparability = (
            pipeline.quadratic_lyapunov_family.comparability()
        )

        pipeline.tool_executor = None

        pipeline.adaptive_kappa_estimator = AdaptiveKappaEffEstimator()
        ...
        pipeline.projection_validator = ProjectionValidator(pipeline.projection_domain)
